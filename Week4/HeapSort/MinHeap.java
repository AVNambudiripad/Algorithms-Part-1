public class MinHeap<T extends Comparable<? super T>>
{
    ...
    private void swim(int index)
    {
        if (index==0)
        return;
        while (index>0 && less(arr[index] , arr[parent(index)] ))
        {
    ...
    private void sink(int index)
    {
        ...
        while (b && leftChild(index)<count )
        {
            ...
            {
                if (less(arr[leftChild(index)],arr[rightChild(index)]))
                {
                    if (less(arr[leftChild(index)],arr[index]))
                    {
                ...
                {
                    if (less(arr[rightChild(index)],arr[index]))
                    {
    ...
    public static void main(String[] args)
    {
        ...
        MinHeap<Integer> obj=new MinHeap<>(a);
        obj.print();
        obj.insert(4);
        obj.insert(-3);
        obj.print();
